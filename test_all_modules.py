...
class ModuleTester:
    ...
    def test_module_methods(self, instance: Any, module_name: str) -> Dict[str, bool]:
        ...
        common_methods = ['search', 'analyze', 'enumerate', 'scrape', 'fetch', 'get_history']
    ...
    def test_module_registry(self) -> bool:
        ...
        try:
            ...
            self.print_success(f"MODULE_REGISTRY loaded with {len(MODULE_REGISTRY)} modules")
            return True
        except Exception as e:
    ...
    def run_all_tests(self):
        ...
        for module_name, class_name in modules_to_test:
            ...
            if self.test_module_import(module_name, class_name):
                ...
                self.results[module_name] = {'import': True, 'instantiate': False, 'methods': {}}
                self.passed_modules += 1
            else:
                self.results[module_name] = {'import': False, 'instantiate': False, 'methods': {}}
            ...
            if self.results[module_name]['import']:
                ...
                if instance:
                    ...
                    if methods:
                        ...
                        self.print_success(f"{module_name} has methods: {', '.join(methods.keys())}")
    ...
    def generate_report(self):
        ...
        success_rate = (self.passed_modules / self.total_modules * 100) if self.total_modules > 0 else 0
        ...
        for module_name, result in sorted(self.results.items()):
            ...
            print(f"  {status} {module_name:<30} | Instantiate: {instantiate} | Methods: {methods_count}")
        
        # Save results to file
        report_file = f"module_test_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        try:
