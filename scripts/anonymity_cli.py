...
from anonymity_grid import (AnonymityGrid, GridNodeRole, QueryPriority,
                            anonymous_query, initialize_anonymity_grid)
        ...
        try:
            while True:
                ...
                if args.show_stats:
                    ...
                    print(f"\rQueries: {stats['stats']['queries_submitted']} | "
                          f"Processed: {stats['stats']['queries_processed']} | "
                          f"Active: {stats['active_queries']}", end="")
...
def cmd_submit_query(args):
    ...
    try:
        priority = (
            QueryPriority(args.priority) if args.priority 
...
def cmd_test_anonymity(args):
    ...
    consumer.add_grid_node("test_mixer", GridNodeRole.MIXER, 
                          {"query_mixing", "bundle_processing"})
    ...
    print(f"Consumer - Submitted: {consumer_stats['stats']['queries_submitted']}, "
          f"Processed: {consumer_stats['stats']['queries_processed']}")
    print(f"Mixer - Mixed: {mixer_stats['stats']['queries_mixed']}, "
...
Examples:
  ...
  %(prog)s sample                                # Create sample queries
    ...
    start_parser.add_argument('--role', default='consumer',
                             choices=[r.value for r in GridNodeRole],
                             help='Node role')
    start_parser.add_argument('--interactive', action='store_true',
                             help='Run in interactive mode')
    start_parser.add_argument('--show-stats', action='store_true',
                             help='Show statistics in interactive mode')
    ...
    query_parser.add_argument('--priority', type=int, choices=[1,2,3,4,5],
    ...
    batch_parser.add_argument('--timeout', type=float, default=120.0,
    ...
    sample_parser = subparsers.add_parser('sample', 
                                        help='Create sample queries file')
