...
def scan_for_hardcoded_secrets() -> List[str]:
    ...
    secret_patterns = [
        (r'(?i)(api[_-]?key|password|secret|token)\s*[:=]\s*["\'][^"\']{20,}["\']', 'Potential hardcoded secret'),
    ...
    exclude_files = {'.git', '__pycache__', 'node_modules', '.env.example', 'security_audit.py'}
        ...
        for file in files:
            ...
            try:
                ...
                for pattern, description in secret_patterns:
                    ...
                    if matches:
                        issues.append(f"{file_path}: {description} - {len(matches)} matches")
...
def check_docker_security() -> List[str]:
    ...
    for docker_file in docker_files:
        ...
        if 'password' in content.lower() and '${' not in content:
            password_lines = [line.strip() for line in content.split('\n') 
                            if 'password' in line.lower() and '${' not in line]
            if password_lines:
                issues.append(f"{docker_file}: Potential hardcoded passwords: {password_lines}")
    ...
    for pattern in log_patterns:
        ...
        for log_file in glob.glob(pattern):
            if os.path.getsize(log_file) > 50 * 1024 * 1024:  # 50MB
                issues.append(f"Large log file detected: {log_file} - may contain sensitive data")
