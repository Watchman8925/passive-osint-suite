...
async def demo_basic_audit():
    ...
    operations = [
        ("domain_lookup", "user", "example.com", 
         {"resolver": "doh", "records": ["A", "MX"]}),
        ("ip_lookup", "user", "8.8.8.8", 
         {"source": "whois", "timeout": 10}),
        ("file_analysis", "media_forensics", "suspect.jpg", 
         {"type": "exif", "size": 1024000}),
        ("port_scan", "network_intel", "192.168.1.1", 
         {"ports": [22, 80, 443], "method": "passive"}),
        ("certificate_check", "ssl_analyzer", "badsite.com", 
         {"protocol": "https", "chain_depth": 3}),
        ("subdomain_enum", "bellingcat_toolkit", "target.com", 
...
def demo_decorator():
    ...
    print(f"✓ Bitcoin analysis: {btc_result['address']} -> "
