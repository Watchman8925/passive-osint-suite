...
class PatentPassive(OSINTUtils):
    ...
    def search_patent_databases(self, query, limit=10):
        ...
        for source_key, source_info in self.sources.items():
            try:
                source_results = self.search_single_patent_source(
                    source_key, query, limit
                )
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                return {
                    ...
                    "patents": patents,
                    "assignees": [
                        p.get("assignee", "") for p in patents if p.get("assignee")
                    ],
                    "inventors": [],
                }
            else:
    ...
    def get_patent_details(self, patent_number, source="google_patents"):
        ...
        if source == "google_patents":
            ...
            return {
                "status": "error",
                "error": f"Detailed search not implemented for {source}",
            }
