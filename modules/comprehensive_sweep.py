...
class ComprehensiveInvestigationSweep(OSINTUtils):
    ...
    def __init__(self):
        ...
        self.core_modules = {
            ...
            "network": ["ip_intel", "passive_dns_enum", "network_analysis"],
            "web": [
                "web_scraper",
                "wayback_machine",
    ...
    def comprehensive_sweep(
        ...
        """
        self.logger.info(
            f"Starting comprehensive investigation sweep for: {target} ({target_type})"
        )
        ...
        self.logger.info(
            f"Selected {len(modules_to_run)} modules for {target_type} investigation"
        )
        ...
        results["execution_stats"] = self._calculate_execution_stats(
            module_results, start_time
        )

        # Extract findings and leads
        findings, leads, pivots = self._process_module_results(
            module_results, target, target_type
        )
    ...
    def _select_modules_for_target(self, target_type: str) -> List[str]:
        ...
        if target_type == "domain":
            return (
                self.core_modules["domain"]
                + self.core_modules["web"]
                + self.core_modules["breach"]
                + ["github_search", "company_intel"]
            )
        elif target_type == "ip":
            return self.core_modules["network"] + ["ip_intel", "passive_dns_enum"]
        elif target_type == "email":
            return (
                self.core_modules["email"]
                + self.core_modules["breach"]
                + self.core_modules["social"]
                + ["paste_site_monitor"]
            )
        elif target_type == "company":
            return (
                self.core_modules["business"]
                + self.core_modules["web"]
                + self.core_modules["financial"]
                + ["social_media_footprint"]
            )
    ...
    def _execute_modules_concurrent(
        self,
        modules: List[str],
        target: str,
        target_type: str,
        ...
        with ThreadPoolExecutor(max_workers=max_concurrent) as executor:
            ...
            for module_name in modules:
                future = executor.submit(
                    self._execute_single_module, module_name, target, target_type
                )
    ...
    def _execute_single_module(
        self, module_name: str, target: str, target_type: str
    ) -> Dict[str, Any]:
    ...
    def _process_module_results(
        self, module_results: Dict[str, Any], target: str, target_type: str
    ):
        ...
        for module_name, result in module_results.items():
            ...
            leads.extend(
                self._extract_leads_from_result(
                    module_name, module_data, target, target_type
                )
            )
            pivots.extend(
                self._extract_pivots_from_result(
                    module_name, module_data, target, target_type
                )
            )
    ...
    def _extract_leads_from_result(
        self, module_name: str, result: Any, target: str, target_type: str
    ) -> List[Dict]:
                ...
                leads.append(
                    {
                        "type": "social_profile",
                        "value": profile.get(
                            "url", profile.get("username", str(profile))
                        ),
                        "source": module_name,
                        "confidence": "medium",
    ...
    def _extract_pivots_from_result(
        self, module_name: str, result: Any, target: str, target_type: str
    ) -> List[Dict]:
    ...
    def _calculate_execution_stats(
        self, module_results: Dict[str, Any], start_time: float
    ) -> Dict[str, Any]:
        """Calculate execution statistics."""
        total_modules = len(module_results)
        successful_modules = sum(
            1 for r in module_results.values() if r.get("success", False)
        )
    ...
    def _verify_opsec_compliance(self, results: Dict[str, Any]) -> str:
        ...
        non_passive_modules = [
            mod for mod in executed_modules if mod not in self.opsec_safe_modules
        ]
    ...
    def generate_investigation_report(self, sweep_results: Dict[str, Any]) -> str:
        ...
        report.append(
            f"**Target:** {sweep_results['target']} ({sweep_results['target_type']})"
        )
            ...
            for finding_type, finding_list in findings.items():
                if finding_list:
                    ...
                    for finding in finding_list[:5]:  # Limit to top 5 per type
                        ...
                        report.append(
                            f"- **{module}:** {str(finding.get('data', ''))[:100]}..."
                        )
                    report.append("")
        ...
        if leads:
            ...
            for lead in leads[:10]:  # Top 10 leads
                report.append(
                    f"- **{lead['type'].upper()}:** {lead['value']} ({lead['confidence']} confidence)"
                )
        ...
        if pivots:
            ...
            for pivot in pivots:
                report.append(
                    f"- **{pivot['type'].replace('_', ' ').title()}:** {pivot['target']}"
                )
