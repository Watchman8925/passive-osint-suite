...
class PatternMatchingEngine:
    ...
    def _check_tool(self, tool_name: str) -> bool:
        ...
        try:
            subprocess.run([tool_name, '--help' if tool_name != 'strings' else '--help'],
                         capture_output=True, timeout=5)
            return True
        except (subprocess.TimeoutExpired, FileNotFoundError, subprocess.CalledProcessError):
            return False

    def yara_scan_file(self, file_path: str, rules_path: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    def yara_scan_directory(self, directory_path: str, rules_path: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    def extract_strings_from_file(self, file_path: str, min_length: int = 4) -> Dict[str, Any]:
        """
                ...
                if matches:
                    ...
                    for match in unique_matches:
                        if isinstance(match, tuple):
                            match = ''.join(match)
                        if len(match) > 10 and len(match) < 200:  # Reasonable length limits
    ...
    def grep_search_patterns(self, search_path: str, patterns: List[str]) -> Dict[str, Any]:
        """
        ...
        for pattern in patterns:
            try:
                ...
                matches = []
                if result.returncode in [0, 1] and result.stdout:  # 0 = matches, 1 = no matches
                    lines = result.stdout.strip().split('\n')
                    for line in lines:
                        if line.strip():
                            matches.append(line.strip())
    ...
    def comprehensive_security_analysis(self, target_path: str) -> Dict[str, Any]:
        ...
        if os.path.isfile(target_path):
            ...
            if file_ext in ['.exe', '.dll', '.bin', '.so', '']:
                analysis["analyses"]["strings"] = self.extract_strings_from_file(target_path)
            ...
            try:
                with open(target_path, 'r', encoding='utf-8', errors='ignore') as f:
                    ...
                    if file_ext in ['.js', '.jsx', '.ts', '.tsx']:
                        analysis["analyses"]["javascript_analysis"] = self.analyze_javascript_file(content)
                    elif file_ext in ['.html', '.htm']:
                        analysis["analyses"]["webpage_analysis"] = self.analyze_webpage_content(content)
                    else:
                        analysis["analyses"]["secrets_scan"] = self.find_secrets_in_text(content)

            except (UnicodeDecodeError, IOError):
                analysis["analyses"]["content_analysis"] = {"error": "Could not read file content"}

        elif os.path.isdir(target_path):
            # Directory analysis
            analysis["analyses"]["yara_directory_scan"] = self.yara_scan_directory(target_path)
            ...
            analysis["analyses"]["sensitive_pattern_search"] = self.grep_search_patterns(
                target_path, sensitive_patterns)
