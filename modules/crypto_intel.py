...
class CryptocurrencyIntelligence(OSINTUtils):
    ...
    def analyze_crypto_address(self, address, currency_type="bitcoin"):
        ...
        self.results = {
            ...
            'clustering_analysis': self.analyze_address_clustering(address, currency_type),
            'exchange_analysis': self.analyze_exchange_connections(address, currency_type),
    ...
    def get_transaction_history(self, address, currency_type):
        ...
        try:
            if currency_type.lower() == "bitcoin":
                ...
                transaction_history = {'error': 'Transaction history not supported for this currency'}
            ...
            if response and response.status_code == 200:
                ...
                for tx in transactions[:20]:  # Limit to 20 recent transactions
                    processed_tx = {
                        ...
                        'total_input': sum([vin.get('prevout', {}).get('value', 0) for vin in tx.get('vin', [])]) / 100000000,
                        'total_output': sum([vout.get('value', 0) for vout in tx.get('vout', [])]) / 100000000
                    }
                ...
                if data.get('status') == '1':
                    ...
                    for tx in transactions:
                        processed_tx = {
                            ...
                            'value_eth': int(tx.get('value', '0')) / 1000000000000000000,
    ...
    def analyze_risk_factors(self, address, currency_type):
        ...
        try:
            ...
            if blacklist_check.get('found_in_blacklists'):
                ...
                risk_analysis['risk_factors'].append('Address found in cryptocurrency blacklists')
            ...
            if 'transactions' in transaction_history:
                ...
                if mixing_patterns:
                    ...
                    risk_analysis['risk_factors'].append('Possible cryptocurrency mixing detected')
            ...
            if exchange_analysis.get('connected_to_exchanges'):
                ...
                risk_analysis['risk_factors'].append('Connected to legitimate exchanges (reduces risk)')
    ...
    def detect_mixing_patterns(self, transactions):
        ...
        try:
            ...
            for tx in transactions:
                ...
                if total_output and (total_output == int(total_output) or 
                                   total_output in [0.1, 0.5, 1.0, 5.0, 10.0]):
    ...
    def analyze_address_clustering(self, address, currency_type):
        ...
        try:
            ...
            if 'transactions' in transaction_history:
                ...
                clustering_analysis['related_addresses'] = list(related_addresses)[:20]  # Limit results
                            ...
                            for pattern in patterns:
                                if pattern in from_addr or pattern in to_addr:
                                    ...
                                    exchange_analysis['exchange_connections'].append({
                                        ...
                                        'direction': 'received' if pattern in from_addr else 'sent'
                                    })
            ...
            if intelx_key:
                ...
                if response.status_code == 200:
                    ...
                    return {
                        ...
                        'sources': [record.get('bucket') for record in results.get('records', [])][:5]
                    }

            return {
                'checked': False,
                'reason': 'No Intel X API key available'
            }
