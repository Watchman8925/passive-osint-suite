...
class PreSeededDatabases(OSINTUtils):
    ...
    def fetch_cisa_vulnerabilities(self) -> List[Dict]:
        ...
        try:
            response = self.make_request(
                self.databases['us_cisa_known_exploited']['url'],
                timeout=30
            )
    ...
    def search_cisa_vulnerabilities(self, query: str) -> List[Dict]:
        ...
        for vuln in vulnerabilities:
            if (query_lower in vuln.get('cveID', '').lower() or
                query_lower in vuln.get('vendorProject', '').lower() or
                query_lower in vuln.get('shortDescription', '').lower()):
                results.append(vuln)
    ...
    def fetch_fbi_most_wanted(self) -> List[Dict]:
        ...
        try:
            response = self.make_request(
                self.databases['us_fbi_most_wanted']['url'],
                timeout=30
            )
            if response and response.status_code == 200:
                data = response.json()
    ...
    def search_fbi_most_wanted(self, name: Optional[str] = None, crime: Optional[str] = None) -> List[Dict]:
    ...
    def fetch_ofac_sanctions(self) -> List[str]:
        ...
        try:
            response = self.make_request(
                self.databases['us_treasury_sanctions']['url'],
                timeout=30
            )
            if response and response.status_code == 200:
                # Parse the SDN list (simplified - in practice this is complex)
                lines = response.text.split('\n')
                sanctions = [line.strip() for line in lines if line.strip() and not line.startswith(';')]
    ...
    def fetch_fema_disasters(self) -> List[Dict]:
        ...
        try:
            response = self.make_request(
                self.databases['us_fema_disaster_declarations']['url'],
                timeout=30
            )
            if response and response.status_code == 200:
                data = response.json()
    ...
    def search_fema_disasters(self, state: Optional[str] = None, disaster_type: Optional[str] = None) -> List[Dict]:
    ...
    def fetch_noaa_weather_alerts(self) -> List[Dict]:
        ...
        try:
            response = self.make_request(
                self.databases['us_noaa_weather_alerts']['url'],
                timeout=30
            )
            if response and response.status_code == 200:
                data = response.json()
    ...
    def fetch_usgs_earthquakes(self) -> List[Dict]:
        ...
        try:
            response = self.make_request(
                self.databases['usgs_earthquake_feed']['url'],
                timeout=30
            )
            if response and response.status_code == 200:
                data = response.json()
    ...
    def fetch_nasa_neo(self) -> List[Dict]:
        ...
        try:
            response = self.make_request(
                self.databases['nasa_neo_feed']['url'],
                timeout=30
            )
            if response and response.status_code == 200:
                data = response.json()
    ...
    def fetch_cdc_alerts(self) -> List[Dict]:
        ...
        try:
            response = self.make_request(
                self.databases['cdc_health_alerts']['url'],
                timeout=30
            )
            if response and response.status_code == 200:
                data = response.json()
    ...
    def comprehensive_search(self, query: str, categories: Optional[List[str]] = None) -> Dict[str, Any]:
        """
