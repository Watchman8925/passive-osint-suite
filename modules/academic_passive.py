...
class AcademicPassive(OSINTUtils):
        ...
        for source_key, source_info in self.sources.items():
            try:
                ...
                if source_results["status"] == "success":
                    ...
                    results["institutions"].extend(
                        source_results.get("institutions", [])
                    )
    ...
    def search_google_scholar(self, query, limit=10):
        ...
        try:
            resp = self.request_with_fallback(
                "get", search_url, timeout=15, allow_fallback=True
            )
            if resp.status_code == 200:
                ...
                return {
                    ...
                    "papers": papers,
                    "authors": [
                        p.get("authors", "") for p in papers if p.get("authors")
                    ],
                    "institutions": [],
                }
            else:
    ...
    def search_semantic_scholar(self, query, limit=10):
        ...
        search_url = (
            f"https://www.semanticscholar.org/search?q={urllib.parse.quote(query)}"
        )
        papers = []
    ...
    def search_pubmed(self, query, limit=10):
        ...
        search_url = (
            f"https://pubmed.ncbi.nlm.nih.gov/?term={urllib.parse.quote(query)}"
        )
        papers = []
