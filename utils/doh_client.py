...
class DoHClient:
    ...
    def _cleanup_cache(self):
        ...
        with self._cache_lock:
            ...
            expired_keys = [
                key for key, record in self._cache.items() if record.is_expired()
            ]
            ...
            if self.rotate_resolvers:
                ...
                while attempts < len(self.resolvers):
                    ...
                    self._current_resolver_index = (
                        self._current_resolver_index + 1
                    ) % len(self.resolvers)
    ...
    async def resolve(
        self, name: str, rtype: str = "A", use_cache: bool = True
    ) -> DoHResponse:
        """
        ...
        for attempt in range(len(self.resolvers)):
            ...
            try:
                ...
                if response.status_code == 200:
                    # Parse DNS response
                    doh_response = self._parse_dns_response(response.content, resolver)
