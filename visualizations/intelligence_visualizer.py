...
class IntelligenceVisualizer:
    ...
    def create_risk_score_gauge(self, risk_score: int, title: str = "Risk Score") -> go.Figure:
    ...
    def create_intelligence_network_graph(self, intelligence_data: Dict) -> go.Figure:
        ...
        for source, data in intelligence_data.items():
            if isinstance(data, dict) and data:
                ...
                G.add_node(source, type='source', size=20, color=self.source_colors.get(source, '#6c757d'))
            ...
            if len(source_list) > 1:
                for i in range(len(source_list)):
                    for j in range(i + 1, len(source_list)):
                        G.add_edge(source_list[i], source_list[j],
                                 weight=1, entity_type=entity_type)
    ...
    def create_timeline_analysis(self, temporal_data: List[Dict]) -> go.Figure:
        ...
        if 'timestamp' not in df.columns:
            df['timestamp'] = pd.date_range(start=datetime.now() - timedelta(days=30),
    ...
    def create_source_contribution_chart(self, source_contributions: Dict) -> go.Figure:
        ...
        colors = [self.source_colors.get(source.split('_')[0], '#6c757d') for source in sources]
    ...
    def create_comprehensive_dashboard(self, intelligence_data: Dict) -> Dict[str, go.Figure]:
        ...
        dashboard['network_graph'] = self.create_intelligence_network_graph(intelligence_data)
