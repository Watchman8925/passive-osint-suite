...
class OSINTDashboard:
    ...
    def setup_layout(self):
        ...
        self.app.layout = dbc.Container([
            ...
            dbc.Row([
                dbc.Col([
                    html.H1("OSINT Intelligence Dashboard",
                           className="text-center text-primary mb-4"),
                    html.P("Comprehensive intelligence analysis and risk assessment platform",
                            ...
                            dbc.Row([
                                dbc.Col([
                                    ...
                                    dbc.Button("Export Report", id="export-btn",
                                             color="secondary", className="mt-3 ml-2")
                                ])
                            ])
                        ])
                    ])
            ...
            dbc.Row([
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H4("Overall Risk Score", className="card-title text-center"),
                            dcc.Graph(id='overall-risk-gauge', style={'height': '300px'})
                        ])
                    ])
                ], width=4),
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H4("Active Intelligence Sources", className="card-title text-center"),
                            html.H2(id='active-sources-count', className="text-center text-primary"),
                            html.P("Sources providing data", className="text-center text-muted")
                        ])
                    ])
                ], width=4),
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H4("High Risk Alerts", className="card-title text-center"),
                            html.H2(id='high-risk-count', className="text-center text-danger"),
                            html.P("Entities requiring attention", className="text-center text-muted")
                        ])
                    ])
                ], width=4)
            ], className="mb-4"),
                ...
                dbc.Col([
                    dbc.Tabs([
                        dbc.Tab([
                            dbc.Row([
                                dbc.Col([
                                    dcc.Graph(id='intelligence-network', style={'height': '500px'})
                                ], width=8),
                                dbc.Col([
                                    dcc.Graph(id='source-contributions', style={'height': '500px'})
                                ], width=4)
                            ])
                        ], label="Intelligence Network", tab_id="network-tab"),

                        dbc.Tab([
                            dbc.Row([
                                dbc.Col([
                                    dcc.Graph(id='timeline-analysis', style={'height': '400px'})
                                ])
                            ]),
                            dbc.Row([
                                dbc.Col([
                        ...
                        ], label="Timeline & Risk Analysis", tab_id="timeline-tab"),

                        dbc.Tab([
                            dbc.Row([
                                dbc.Col([
                                    dcc.Graph(id='transaction-flow', style={'height': '500px'})
                                ], width=6),
                                dbc.Col([
                                    dcc.Graph(id='balance-timeline', style={'height': '500px'})
                                ], width=6)
                            ]),
                            dbc.Row([
                                dbc.Col([
                                    dcc.Graph(id='wallet-risk-heatmap', style={'height': '400px'})
                                ])
                            ])
                        ], label="Financial Analysis", tab_id="financial-tab"),

                        dbc.Tab([
                            dbc.Row([
                                dbc.Col([
                                    dcc.Graph(id='geographic-map', style={'height': '500px'})
                                ])
                            ])
                        ], label="Geographic View", tab_id="geographic-tab")
                    ], id="main-tabs", active_tab="network-tab")
                ])
            ], className="mb-4"),
                                ...
                                children=[
                                    dash_table.DataTable(
                                        ...
                                        style_data_conditional=[
                                            ...
                                            {
                                                'if': {'column_id': 'risk_score', 'filter_query': '{risk_score} > 60 && {risk_score} <= 80'},
            ...
            dbc.Row([
                dbc.Col([
                    html.Hr(),
                    html.P("OSINT Intelligence Dashboard - Real-time analysis and risk assessment",
        ...
        ], fluid=True, style={'backgroundColor': '#1a1e22'})
    ...
    def setup_callbacks(self):
        ...
        def update_dashboard(n_clicks, active_tab):
            ...
            network_graph = self.intel_visualizer.create_intelligence_network_graph(intelligence_data)

            source_contributions = self.intel_visualizer.create_source_contribution_chart(
            ...
            transaction_flow = self.financial_visualizer.create_transaction_flow_diagram(
            ...
            # Calculate metrics
            active_sources = len([k for k, v in intelligence_data.items() if isinstance(v, dict) and v])
            high_risk_count = len([r for r in intelligence_data.get('risk_scores', [])
            ...
            return (
                risk_gauge, active_sources, high_risk_count,
                network_graph, source_contributions, timeline, risk_dist,
                transaction_flow, balance_timeline, wallet_heatmap, geo_map,
    ...
    def create_sample_data(self) -> Dict:
        ...
        return {
            ...
            'risk_scores': [
                {'entity': f'Entity_{i}', 'risk_score': 20 + i * 3,
            ...
            'financial': {
                ...
                'wallets': [
                    {'name': f'Wallet_{i}', 'risk_score': 20 + i * 3}
                    for i in range(10)
                ]
            },
            'geo_data': [
                {'lat': 40.7128 + i * 0.1, 'lon': -74.0060 + i * 0.1,
                 'name': f'Location_{i}', 'risk_score': 30 + i * 5,
...
def create_dashboard(data_manager=None, host: str = '0.0.0.0', port: int = 8050, debug: bool = False):
