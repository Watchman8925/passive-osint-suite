...
class EnhancedReportGenerator:
    ...
    def generate_user_friendly_report(
        ...
        report = {
            ...
            'investigation_name': investigation_data.get('name', 'Unnamed Investigation'),
            ...
            'what_we_think': self._generate_analysis_section(
                findings, analysis
            ),
            ...
            'why_it_matters': self._explain_significance(
...
Status: {len([l for l in leads if l.get('status') == 'pending'])} leads pending investigation, 
{len([l for l in leads if l.get('status') == 'completed'])} already explored.
        """.strip()
    ...
    def _assess_risks(
        ...
        if breach_count > 0:
            ...
            risk_factors.append(f"Found {breach_count} breach exposure(s) - credentials may be compromised")
        ...
        admin_findings = [f for f in findings if 'admin' in str(f.get('value', '')).lower()]
        if admin_findings:
            risk_score += len(admin_findings) * 3
            risk_factors.append(f"{len(admin_findings)} administrative interfaces found")
    ...
    def _generate_recommendations(
        ...
        high_value_leads = [l for l in leads if l.get('estimated_value') == 'high' and l.get('status') == 'pending']
        if high_value_leads:
            recommendations.append({
                'priority': 'high',
    ...
    def _calculate_statistics(
        ...
        return {
            ...
            'completed_leads': len([l for l in leads if l.get('status') == 'completed']),
    ...
    def _explain_finding_value(self, finding_type: str, value: str) -> str:
        ...
        explanations = {
            'email': f"Email address that can be checked for breaches and linked accounts",
            'domain': f"Website or service domain that can be further investigated",
            'ip': f"Server IP address that can reveal hosting and services",
            'subdomain': f"Additional web property that may host services",
            'breach': f"Data leak containing potentially sensitive information",
            'username': f"User identifier that may appear on multiple platforms"
        }
        return explanations.get(finding_type, "Data point discovered during investigation")
    ...
    def _explain_lead_importance(self, lead: Dict[str, Any]) -> str:
        ...
        return importance_map.get(target_type, "May provide valuable investigation intelligence")
    ...
    def _predict_potential_findings(self, lead: Dict[str, Any]) -> List[str]:
        ...
        predictions = {
            'email': ['Breach exposure', 'Social media accounts', 'Associated services'],
            'domain': ['Subdomains', 'DNS records', 'SSL certificates', 'Server information'],
            'ip': ['Open ports', 'Hosted services', 'Geolocation', 'Network information'],
    ...
    def _identify_patterns(self, findings: List[Dict[str, Any]]) -> List[str]:
        ...
        # Domain patterns
        domains = [f.get('value', '') for f in findings if f.get('finding_type') == 'domain']
        if len(domains) > 3:
            patterns.append(f"Multiple domains identified ({len(domains)}), suggesting complex infrastructure")
        
        # Email patterns
        emails = [f.get('value', '') for f in findings if f.get('finding_type') == 'email']
        if emails:
            email_domains = set(e.split('@')[1] for e in emails if '@' in e)
            if len(email_domains) == 1:
                patterns.append(f"All emails from same domain, indicating centralized organization")
    ...
    def _identify_connections(self, findings: List[Dict[str, Any]]) -> List[str]:
        ...
        if related_findings:
            connections.append(f"{len(related_findings)} findings have identified relationships")
    ...
    def _identify_anomalies(self, findings: List[Dict[str, Any]]) -> List[str]:
        ...
        if low_confidence:
            anomalies.append(f"{len(low_confidence)} findings have low confidence - require verification")
...
and informed decision-making about protective measures.
    ...
    def _calculate_progress(
        ...
        if not leads:
            return 100.0
        completed = len([l for l in leads if l.get('status') == 'completed'])
