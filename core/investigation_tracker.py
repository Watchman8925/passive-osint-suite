...
class InvestigationTracker:
    ...
    def _init_database(self):
        ...
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_findings_value ON findings(value)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_leads_investigation ON leads(investigation_id)')
    ...
    def link_findings(
        ...
        for finding_id, related_id in [(finding_id_1, finding_id_2), (finding_id_2, finding_id_1)]:
            cursor.execute('SELECT related_findings FROM findings WHERE id = ?', (finding_id,))
            row = cursor.fetchone()
            if row:
    ...
    def get_investigation_summary(
        ...
        # Get summary
        cursor.execute('SELECT * FROM investigation_summary WHERE investigation_id = ?', (investigation_id,))
        row = cursor.fetchone()
        ...
        ''', (investigation_id,))
        findings_by_type = {row[0]: {'count': row[1], 'avg_confidence': row[2]} for row in cursor.fetchall()}
        ...
        cursor.execute('''
            ...
            GROUP BY priority
        ''', (investigation_id,))
        leads_by_priority = {row[0]: {'total': row[1], 'completed': row[2]} for row in cursor.fetchall()}
    ...
    def add_timeline_event(
        ...
        cursor.execute('SELECT timeline FROM investigation_summary WHERE investigation_id = ?', (investigation_id,))
        row = cursor.fetchone()
            ...
            with open(filepath, 'w') as f:
                ...
                for finding_type, findings_list in report['findings_by_type'].items():
                    ...
                    for finding in findings_list[:10]:  # Top 10
                        f.write(f"- **{finding['value']}** (Source: {finding['source']}, Confidence: {finding['confidence']:.2f})\n")
                    f.write("\n")
                
                f.write(f"## High Priority Leads\n\n")
