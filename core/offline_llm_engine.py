...
class OfflineLLMEngine:
    ...
    def analyze_investigation_data(
        self, investigation_data: Dict[str, Any], focus: str = "comprehensive"
    ) -> AnalysisResult:
        """
        ...
        prompt = self._build_analysis_prompt(
            targets, results, investigation_type, focus
        )
...
"""
    ...
    def _format_leads(self, entities: List[Dict[str, str]]) -> str:
        ...
        for entity in entities[:5]:
            ...
            if entity_type == "email":
                leads.append(
                    f"- Email: {value} - Check breach databases and social profiles"
                )
            elif entity_type == "domain":
                leads.append(
                    f"- Domain: {value} - Enumerate subdomains and check DNS records"
                )
            elif entity_type == "ip":
    ...
    def _parse_analysis(
        self, analysis_text: str, investigation_data: Dict[str, Any]
    ) -> AnalysisResult:
        """Parse analysis text into structured result"""

        # Extract sections
        summary = self._extract_section(analysis_text, ["executive summary", "summary"])
        findings = self._extract_list_section(
            analysis_text, ["key findings", "findings"]
        )
        actions = self._extract_list_section(
            analysis_text, ["next steps", "recommended", "actions"]
        )
    ...
    def _generate_lead_reason(self, entity: Dict[str, str]) -> str:
        ...
        return reasons.get(
            entity_type, "Entity may provide valuable intelligence connections"
        )
    ...
    def explain_finding(
        self, finding: Dict[str, Any], context: str = ""
    ) -> Dict[str, Any]:
        """
    ...
    def _describe_finding(self, finding: Dict[str, Any]) -> str:
        ...
        return descriptions.get(
            finding_type, f"Finding of type '{finding_type}' identified"
        )
    ...
    def _explain_significance(self, finding: Dict[str, Any]) -> str:
        ...
        return significance.get(
            finding_type, "This finding may provide additional investigation leads"
        )
    ...
    def _suggest_next_steps_for_finding(self, finding: Dict[str, Any]) -> List[str]:
        ...
        return next_steps_map.get(
            finding_type, ["Investigate further using related modules"]
        )
